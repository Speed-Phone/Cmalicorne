...
export default function Boutique() {
  ...
  // --- FONCTION MAGIQUE POUR NETTOYER LE LIEN ---
  const nettoyerImage = (url: string) => {
    if (!url) return "/t-shirt.png"; // Image de secours
    // 1. On enlève tous les espaces
    let propre = url.trim();
    // 2. On enlève le slash au début s'il y en a un pour ne pas en avoir deux
    propre = propre.replace(/^\/+/, '');
    // 3. On rajoute UN SEUL slash propre au début
    return `/${propre}`;
  };
              ...
              <div key={p.id} className="group cursor-pointer">
                <div className="relative overflow-hidden rounded-[2rem] aspect-[3/4] mb-5 shadow-sm group-hover:shadow-2xl transition-all duration-500 border border-purple-50">
                  ...
                  <div className="absolute inset-0 bg-black/70 backdrop-blur-[3px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center p-6">
                    <p className="text-white text-[10px] font-black uppercase tracking-[0.3em] mb-4 text-center">Choisir Taille</p>
                    <div className="flex flex-wrap justify-center gap-2">
                      {['S', 'M', 'L', 'XL'].map((t) => (
                        <button 
                          key={t}
                          onClick={() => ajouterAuPanier({id: p.id, name: p.nom, price: p.prix, image: imagePropre}, t)}
